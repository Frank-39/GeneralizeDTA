...
class fc(nn.Module):
    def __init__(self,in_dim, n_hidden_1, n_hidden_2,out_dim,dropout=0.1):
...
class MetaGCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, weight, bias, edge_encoder_w, edge_encoder_b, edge_weight=None):
        ...
        # print('初始edge_attr,edge_index,x',edge_attr.size(),edge_index.size(),x.size())
        edge_index, edge_weight = add_remaining_self_loops(edge_index, num_nodes = x.size(0))# 这里加了num_nodes = x.size()
        # print(edge_index.size())
        self_loop_attr = torch.zeros(x.size(0), self.edge_in_channels)

        self_loop_attr[:, 1] = 1  # for chem dataset
        self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
